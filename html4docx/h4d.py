...
class HtmlToDocx(HTMLParser):
    ...
    def handle_img(self, current_attrs):
        ...
        height = utils.unit_converter(utils.add_px(current_attrs["height"])) if "height" in current_attrs else None
        width = utils.unit_converter(utils.add_px(current_attrs["width"])) if "width" in current_attrs else None
        ...
        for cell_row, row in enumerate(self.get_table_rows(table_soup)):
            ...
            for col in self.get_table_columns(row):
                ...
                if rowspan > 1 or colspan > 1:
                    docx_cell = docx_cell.merge(self.table.cell(current_row + (rowspan - 1), current_col + (colspan - 1)))
    ...
    def handle_link(self, href, text, tooltip=None):
        ...
        if is_external:
            ...
            rel_id = self.paragraph.part.relate_to(href, docx.opc.constants.RELATIONSHIP_TYPE.HYPERLINK, is_external=True)
    ...
    def _resolve_custom_style(self, tag, attrs):
        ...
        custom_style = self.get_word_style_for_element(tag, attrs) if (self.use_styles or self.use_tag_overrides) else None
        ...
        if tag == "div":
            ...
            if self.div_paragraphs:
                ...
                for para in div_paras:
                    has_own_styles = (hasattr(para, "_has_own_styles") and para._has_own_styles) or (
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--bs", action="store_true", help="Attempt to fix html before parsing. Requires bs4. Default True")
